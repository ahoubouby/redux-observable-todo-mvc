...
function postReq(path, data, headers = { "Content-Type": "application/json" }) {
  return ajax.post(`${BASE_URL}${path}`, data, headers);
}
const createErrorAction = (message) => (error) =>
  Observable.of({
...
const addTodo = (action$) =>
  action$.ofType(TODOS_ADD_REQUEST).mergeMap((action) =>
    postReq("todos", {
      text: action.text,
      completed: false,
    })
...
const removeTodo = (action$) =>
  action$.ofType(TODOS_REMOVE_REQUEST).mergeMap((action) =>
    ajax
...
const completeTodo = (action$) =>
  action$.ofType(TODOS_COMPLETE_REQUEST).mergeMap((action) =>
    ajax
      .patch(
        `http://localhost:3001/todos/${action.id}`,
        {
      ...
      .catch(
        createErrorAction(`Failed to mark task #${action.id} as completed`)
      )
  );

const removeCompletedTodos = (action$, { getState }) =>
  action$.ofType(TODOS_REMOVE_COMPLETED_REQUEST).mergeMap(() =>
    Observable.forkJoin(
      ...getState()
        .todos.data.filter((todo) => todo.completed)
        .map((todo) => ajax.delete(`http://localhost:3001/todos/${todo.id}`))
    )
...
const editTodo = (action$) =>
  action$.ofType(TODOS_EDIT_REQUEST).mergeMap((action) =>
    ajax
      .patch(
        `http://localhost:3001/todos/${action.id}`,
        {
